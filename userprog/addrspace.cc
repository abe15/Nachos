...
void AddrSpace :: clearPhysicalPage(int x)
{
	pageBitMap -> Clear(pageTable[x].physicalPage);
}
...
AddrSpace::~AddrSpace()
{
	...
	for (unsigned int x=0; x< numPages; x++)
		pageBitMap -> Clear(pageTable[x].physicalPage);		//Sets all the used pages to unused before deleting the 									//pageTable for the process
...
//	printf("#########################");
    machine->pageTable = pageTable;
    machine->pageTableSize = numPages+8*MAX_THREADS_PER_PROCESS;
}
