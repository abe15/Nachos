...
// A Lock can have either Free Status or Busy Status
...
class Lock {
  ...
  private:
	...
	LockStatus status;		// is the Lock Free or Busy?

	Thread *ownerThread;		// new Thread instantiated 
					// denotes the thread which owns the lock right now

	List *waitqueue;		// waitqueue denotes the waiting queue for each lock	
	/**********CREATED ENDS HERE*************/
...
class Condition {
  ...
  private:
	...
	Lock *waitLock;			//Wait Lock for the Condition Variable
					//Set to Null initially &  when no one is waiting, i.e. in the queue
					//else set to the Lock passed as argument in Wait, Signal or Broadcast

	List *CVwaitqueue;		// CVwaitqueue denotes the waiting queue for each Condition Variable
