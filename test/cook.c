...
void main()
{
	...
	Acquire(cookIDGeneratorLock);
	c=cookIDGenerator++;		
	Release(cookIDGeneratorLock);

	Coo[c].cookid=c;			/*assigning an id to the cook.*/
	Coo[c].Status= 0;	
	
	Acquire(cookManagerLock);		/*acquiring the cook and manager lock.*/
	
	Wait(cookMCV,cookManagerLock);	/*Initiating the condition variable and waiting ofr the signal from manager.*/
