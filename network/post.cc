...
{
//Error Handling Part for Project 4 Part 3 STARTS HERE
	
	messageCounter=0;
//Error Handling Part for Project 4 Part 3 ENDS HERE
...
int PostOffice::GetMachineID()
{
	return netAddr;
}

//----------------------------------------------------------------------
// PostOffice::ErrorFreeSend
// 	Concatenate the MailHeader to the front of the data, and pass 
//	the result to the Network for delivery to the destination machine.
//
//	Note that the MailHeader + data looks just like normal payload
...
bool PostOffice::ErrorFreeSend(PacketHeader pktHdr, MailHeader mailHdr, char* data)
{
	...
	//Error handling code's declarations end here
	
	//Filling up the variables
	/*
	toMachineID=pktHdr.to;
	toMailboxNO=mailHdr.to;
	
	fromMachineID=netAddr;
	fromMailboxNO=mailHdr.from;
	*/
	
	requestedMessage = strtok(data,"@");
	request[reqCtr] = requestedMessage;
			
	seqNO=atoi(request[reqCtr]);
	
	//Information extracted from the sent packet
	
	for(int m=0; m < messageCounter; m++)
			if (sendMessageDB[m].toMachineID == toMachineID && sendMessageDB[m].toMailboxNO == toMailboxNO 
				&& sendMessageDB[m].fromMachineID == fromMachineID && sendMessageDB[m].fromMailboxNO == fromMailboxNO
					&& sendMessageDB[m].seqNO == seqNO)
	...
	if (! alreadyExists)
	{
		...
		int m=messageCounter;
		
		sendMessageDB[m].toMachineID = toMachineID;
		sendMessageDB[m].toMailboxNO = toMailboxNO;
		sendMessageDB[m].fromMachineID = fromMachineID;
		sendMessageDB[m].fromMailboxNO = fromMailboxNO;
		sendMessageDB[m].seqNO = seqNO;
		
		strcpy(sendMessageDB[m].message, data);
	}
...
void
PostOffice::ErrorFreeReceive(int box, PacketHeader *pktHdr, MailHeader *mailHdr, char* data)
{
		char *requestedMessage;
	char *request[20];
	int reqCtr=0;
...
/*	
	...
	*/
	...
	for(int m=0; m < messageCounter; m++)
			if (receiveMessageDB[m].toMachineID == toMachineID && receiveMessageDB[m].toMailboxNO == toMailboxNO 
				&& receiveMessageDB[m].fromMachineID == fromMachineID && receiveMessageDB[m].fromMailboxNO == fromMailboxNO
					&& receiveMessageDB[m].seqNO == seqNO)
	...
	if (! alreadyExists)
	{
		...
		receiveMessageDB[m].toMachineID = toMachineID;
		receiveMessageDB[m].toMailboxNO = toMailboxNO;
		receiveMessageDB[m].fromMachineID = fromMachineID;
		receiveMessageDB[m].fromMailboxNO = fromMailboxNO;
		receiveMessageDB[m].seqNO = seqNO;
		
		strcpy(receiveMessageDB[m].message, data);
	}
