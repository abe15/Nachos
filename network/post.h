...
//For Error handling part of Project 4 STARTS HERE
struct messageStruc
{
	int seqNO;
...
//For Error handling part of Project 4 ENDS HERE
...
class PostOffice {
  public:
	...
	//Project 3 Error Handling Part
	int messageCounter;

	struct messageStruc sendMessageDB[20];
	struct messageStruc receiveMessageDB[20];
...
//	_____________________Lost Packet Code Starts Here_____________________________________________________
	
	
	bool ErrorFreeSend(PacketHeader pktHdr, MailHeader mailHdr, char *data);
    				// Send a message to a mailbox on a remote 
					// machine.  The fromBox in the MailHeader is 
					// the return box for ack's.

	void ErrorFreeReceive(int box, PacketHeader *pktHdr, MailHeader *mailHdr, char *data);
    				// Retrieve a message from "box".  Wait if
					// there is no message in the box.
								
				
//	_____________________Lost Packet Code Ends Here_____________________________________________________
	...
	int GetMachineID();		//Returns the network address of the current instance, i.e. the machine ID
